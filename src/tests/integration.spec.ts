...
describe("Planets screen components tree", () => {
    ...
    test("Renders list of the planets without the planets containing unknown population data", async () => {
        ...
        const {container} = render(ScreenPlanets);
        const populationFilteringInput =
            container.querySelectorAll('.commonCheckbox__container')[0];

        await fireEvent.click(populationFilteringInput as Element)

        await waitFor(async () => {
            expect((populationFilteringInput.querySelector('.commonCheckbox__input') as HTMLInputElement).checked).toBe(true)
            expect(container.querySelectorAll(".planetRow__container")).toHaveLength(
                17,
            );
        })
    ...
    test("Renders list of the planets without the planets containing unknown population and rotation period data", async () => {
        ...
        await fireEvent.click(container.querySelectorAll('.commonCheckbox__container')[0] as Element)
        await fireEvent.click(container.querySelectorAll('.commonCheckbox__container')[1] as Element)

        await waitFor(async () => {
            expect((container.querySelectorAll('.commonCheckbox__container')[0]
                .querySelector('.commonCheckbox__input') as HTMLInputElement).checked).toBe(true)
            expect((container.querySelectorAll('.commonCheckbox__container')[1]
                .querySelector('.commonCheckbox__input') as HTMLInputElement).checked).toBe(true)
            expect((container.querySelectorAll('.commonCheckbox__container')[2]
                .querySelector('.commonCheckbox__input') as HTMLInputElement).checked).toBe(false)
            expect(container.querySelectorAll(".planetRow__container")).toHaveLength(
                16,
            );
        })
    ...
    test("Renders list of the planets without the planets containing unknown population, rotation period and climate data", async () => {
        ...
        await fireEvent.click(container.querySelectorAll('.commonCheckbox__container')[0] as Element)
        await fireEvent.click(container.querySelectorAll('.commonCheckbox__container')[1] as Element)
        await fireEvent.click(container.querySelectorAll('.commonCheckbox__container')[2] as Element)
